...
- severity: high
- evidence: `src/module/rolls/action-data.mjs:144-156` sets up the `attacker` and `defender` objects for `getOpposedDegreesWithTiebreak` using the raw `dos` from the dice roll (`this.rollData.dos` and `this.rollData.opposedDos`). The `dos` is never adjusted for the Unnatural Characteristic multiplier. (This is also missed in `src/module/rules/grapple.mjs:55` and `26`).
...
- area: armour / rules
- severity: P0 (wrong result in play)
- evidence: `src/module/documents/acolyte.mjs` — inside `_computeArmour()`, the `reduce` loop checks `if (armourVal > acc[location])` to establish `maxArmourCraft[location]`. If the actor equips an AP 3 Common item and an AP 3 Best item, and the Common item is processed first, `3 > 3` evaluates to false, leaving `maxArmourCraft` as 'Common' and ignoring the Best item entirely.
...
- fix (re-fix): Replaced the second-pass `_computeArmour()` rebuild with a new minimal helper `_refreshArmourToughnessBonus()` (`acolyte.mjs:95`, method at `:750-762`) that updates ONLY the cached per-location `toughnessBonus` field from the post-AE `characteristics.toughness.bonus`, leaving the rest of `system.armour` (including any AE-modified `value`/`total`) intact — exactly the first alternative the dispute prescribed. Gate green (`build:check` exit 0; 283 node tests). Live-verified on rt-smoke (`tests/e2e/verify_bug_q_245.py`): an acolyte TB 4 with a -20-Toughness AE + a +5 `system.armour.body.value` AE → post-AE TB 2, `armour.body.toughnessBonus` = 2 (was stale 4; original bug fixed) AND `armour.body.value` = 5 (AE-modified AP preserved, no longer wiped — dispute resolved).
...
- fix (re-fix): Extracted the idempotent Initiative block out of `_computeCharacteristics()` into a new `_computeInitiative()` method (`acolyte.mjs:448`), and `prepareData` now calls it a THIRD time (`acolyte.mjs:103`) after the post-AE `_computeEncumbrance()`, so `initiative.bonus` picks up the refreshed `encumbered` flag. Chose a targeted helper over re-running the whole `_computeCharacteristics()` (the dispute's suggestion) because that method's `wounds.max = woundsMax(wounds.max, modifier)` folds `wounds.modifier` back INTO its own input — a third full run would double-count AE wound bonuses (e.g. Sound Constitution). `_computeInitiative()` only writes `initiative.bonus`, so it is safe to re-run. Gate green (`build:check` exit 0; 283 node tests). Live-verified on rt-smoke (`tests/e2e/verify_bug_q_246.py`): actor AgB 4 carrying 25kg → baseline not-encumbered, `initiative.bonus` = 4; after a -30 STR/-30 TON AE → encumbered = true, AgB still 4, `initiative.bonus` = 3 (the -1 encumbered penalty now applies; pre-fix it stayed 4).
...
- verify: disputed: flawed premise. The cited rule "add another full damage roll for the weapon to the total" from the core rulebook was explicitly removed by the official errata. `Rogue Trader Errata v. 1.4 WQ.pdf/markdown.md:277` states verbatim: "Righteous Fury (page 245): The final sentence of this paragraph reads 'If that second attack hits, the attacker may make an additional damage roll and add it to the damage total.' It should read 'If that second attack hits, the attacker may roll one additional Damage die (1d10) and adds the result to the Damage total.'" Furthermore, the "Psychic powers that deal damage" quote does not exist anywhere in RT-DOCS. Because Righteous Fury is definitively changed to a single 1d10 die by the errata, a correct fix must change the RF `extra` roll to simply be `1d10` for ALL weapons and powers rather than trying to salvage the weapon's `rollFormula` or power's `perDoSFormula`.
