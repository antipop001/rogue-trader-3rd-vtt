...
**rules** (18)
- 🔨 QA-071 (partial 2026-06-25) — Combat-action side-effects. **Done (the self-state "until your next turn" riders):** **All Out Attack** now forfeits Dodge/Parry Reactions — a `system.combat.allOutAttack` flag is set when the action resolves and gates `rollSkill` (the Reaction is no longer freely spendable that Round); **Guarded Attack** grants +10 to Dodge/Parry via a `system.combat.guardedAttack` flag → `modifiers['Guarded Attack']`. Both flags clear at the start of the actor's turn (`onCombatTurnChange`). Schema fields added to `template.json`; live-verified on rt-smoke (blocked Dodge doesn't spend, normal Dodge spends, flags clear on turn). **Deferred (opponent-facing riders — need the target-condition/status modeling): Defensive Stance +1 Reaction & −20 WS to opponents; Charge/Run "+20 WS to attackers in melee".**
- ✅ QA-078 (fixed 2026-06-25) — Encumbered penalties now applied (RT Core p.249): an Encumbered character reduces his Agility Bonus by 1 for **movement rates** (`_computeMovement`) and **Initiative** (reduced at the AgB input so Lightning Reflexes doubles the already-reduced bonus), and takes **−10 to movement-related skill tests** (Dodge/Acrobatics/Climb/Swim/Contortionist via `MOVEMENT_SKILLS` in `rollSkill`). The flag was set but had zero consumers. Live-verified on rt-smoke (AgB 4 → half-move 4→3, full 8→6, Initiative 4→3 when encumbered).
- 🔨 QA-080 (inc.1-4 landed 2026-06-25) — Condition/status-effect layer. **inc.1** registered RT conditions in `CONFIG.statusEffects` + to-hit read. **inc.2** outcomes now attach `[Apply: X]` buttons (Knock Down→Prone, Stun→Stunned, Flame→On Fire) that toggle the status on the target (GM/owner-clicked, like Assign Damage). **inc.3** recurring On Fire/Blood Loss handler re-keyed onto `actor.statuses` (was the orphaned legacy `Burning`/`Bleeding` names — see QA-095); On Fire self-clears on a successful WP test. **inc.4** Stunned/Helpless/Unconscious lock out Dodge/Parry Reactions (`reactionsLocked`). **inc.2b (2026-06-25):** damage-card path done too — weapon specials Concussive→Stunned/Prone, Shocking→Stunned, Flame→On Fire surface `[Apply: X]` buttons on the hit (test-gated, GM adjudicates then clicks); **Critical-Hit effects auto-apply** (deterministic, no test) via `conditionsFromCriticalText()` scanning the crit prose in `assign-damage-data.performActionAndSendToChat` (GM-side, live actor, BUG-005-safe) — Blood Loss / Stunned / Prone / Blinded / On Fire etc. land automatically when the crit is assigned. Remaining: Pinned enforcement (QA-082), status durations not modelled (GM clears via token HUD).
- ✅ QA-081 (fixed 2026-06-25) — Fear / Shock subsystem. New `src/module/rules/fear.mjs`: `rollFearTest(actor, rating, {combat})` runs a Willpower Fear Test at the Table 10-3 modifier (`fearTestModifier`: Fear 1/2/3/4 → 0/−10/−20/−30), with the Insanity auto-immunity (`fearAutoImmune`: tens-digit of Insanity ≥ 2×rating → no test). Combat failure draws the orphaned `Fear` Shock RollTable at **+10×DoF** (`shockRollModifier`, every DoF per RT not "after the first" — BUG-001 family) via `getResultsForRoll`; non-combat failure is −10 concentration, and fail-by-30 (3+ DoF) awards +1d5 Insanity. Surfaced as a "Fear Test" actor-sheet header control + `game.rt.fearTest()` dialog. Pure helpers node-tested; auto-immune / pass / combat-fail-Shock / non-combat-Insanity all live-verified on rt-smoke.
- ✅ QA-082 (fixed 2026-06-25, QA-080 inc.2) — Pinning subsystem. **(A)** A Pinned shooter now takes −20 to all BS (ranged) tests, enforced in the to-hit calc via `attackerConditionModifier(sourceActor.statuses, isRanged)` (reads the ATTACKER's own conditions, vs the target-reading `conditionToHitModifier`). **(B)** Suppressing Fire (Semi −10 / Full −20) and Overwatch (+0) outcomes now surface a "Roll Pinning Test (−X)" button that rolls the target's Willpower at the action difficulty and `toggleStatusEffect('pinned')` on failure (`_rollPinningTest`, GM/owner-clicked). Pure helper node-tested; both parts live-verified on rt-smoke. **Remaining (manual, no action-economy engine): Half-Action-only restriction is documented not enforced; auto-escape (WP test at end of Turn, Easy +30 if no longer under fire; melee auto-escapes) — GM clears the `pinned` status via token HUD.**
- ✅ QA-083 (fixed 2026-06-25) — Insanity / Corruption degradation tracks. New `src/module/rules/degradation.mjs`: pure track helpers (`insanityTrack` 10-5, `corruptionTrack` 10-7), `thresholdsCrossed` / `disordersGained`, and the flows `rollTraumaTest` (WP at Insanity-Track mod → Mental Traumas at +10×DoF), `rollMalignancyTest` (WP at Corruption-Track mod → Malignancies), `rollMutationTest` (every 30 CP → Mutations). A `preUpdateActor`/`updateActor` hook pair auto-fires the tests for every 10-IP / 10-CP / 30-CP boundary crossed (first-GM only), plus Minor/Severe/Acute Disorder notices at IP 40/60/80. Authored the 3 missing RollTables into `tables` pack (Mental Traumas 10-6, Malignancies 10-8, Mutations 14-3 — logged to data-vendor-queue for RT-rules review; Table 14-3 OCR ranges normalised, deep 100+ exotics omitted). Pure helpers node-tested (183 pass); tables-present + threshold message-counts + live-hook auto-fire all verified on rt-smoke. **Simplification: Mutation Test presents the drawn mutation for the GM to apply after the player's two-Characteristic test (player-choice, not auto-applied); item-type packs (malignancy/mutation/mentalDisorder) still unpopulated — tables drive it as text.**
...
- ✅ QA-094 (fixed 2026-06-25) — Degrees of Success/Failure now use the canon **band method** (RT Core p.22): `degreesOfSuccess(target, roll) = floor((target−roll)/10)` (+ symmetric DoF), replacing the DH2 `1 + tens-digit-diff` that over-counted by 1–2 on every successful test. Shared by skill/characteristic/combat (all route through `ActionData.calculateSuccessOrFailure`), the opposed `rollCheck`, and the Acquisition card. **Dependent hit-count formulas re-derived against canon:** Full Auto +1 hit per DoS (`dos`), Semi-Auto +1 per 2 DoS (`floor(dos/2)`); Accurate (`min(floor(dos/2),2)`), Razor Sharp (`dos≥3`), Eye of Vengeance (`=dos`) become correct automatically once DoS is canon. **`getOpposedDegrees` made success-aware** (a bare success is now 0 DoS, so `dos>0` no longer means "succeeded"). Edge qualities with no RT canon (Twin-Linked, melee-Lance) keep their formula shape consuming the corrected DoS (documented). Pure helpers node-tested (Yolanda 18 vs 42 → 2); DoS values + Full/Semi hit counts live-verified on rt-smoke (2 DoS → 3 Full-Auto hits; bare success → 0 DoS).
...
- ✅ QA-113 (fixed 2026-06-25) — Dodge/Parry Reactions integrated into attack resolution (RT Core p.238/242/246). The attacker's result card now shows **Dodge** and **Parry** buttons (when the attack hit a target); clicking rolls the defender's Reaction via the new `acolyte.rollReaction(type)` — which spends the per-Round budget and honours the lockouts (Stunned/Helpless via `reactionsLocked`, All Out Attack via the QA-071 flag, budget via `canSpendReaction`) and adds the Guarded Attack +10. On success it **negates the hit(s)** — disables the assign-damage button(s) on the card (Dodge negates 1 + DoS per RT Core p.238; Parry negates 1) and posts an authoritative "negated, do not assign damage" result; on failure "the hit stands". `rollReaction` budget-spend + all four lockout/blocked paths live-verified on rt-smoke. **Limitation: quick-reaction Parry uses the flat skill value — weapon Parry modifiers (Balanced/Defensive) still need the full sheet Parry control; the assign-damage disable is a live-card affordance (the chat result is the persistent authority).**
...
**ship** (7)
- ✅ QA-042 (fixed 2026-06-26) — Hull damage now = the salvo's combined rolled Damage minus the facing Armour (RT Core p.216): `calculatePenetration` rolls `1d10 + bonus` per non-shielded hit, sums them, and (macrobattery) subtracts the facing Armour ONCE via the node-tested `voidshipHullDamage()` helper; lances sum and ignore Armour. The combined total is carried on the salvo's primary hit and applied in `finalize()` (a Critical that did 0 Hull still inflicts 1 automatic point). Replaces the homebrew penetration-tier comparison + fixed 1/2/4 hull constants. Live-verified (macro Σ−Armour, lance ignore-Armour, hull applied, crit auto-point).
...
- ✅ QA-045 (fixed 2026-06-26) — Void shields no longer drain permanently. `calculateVoidShields` cancels up to `strength` hits from each incoming salvo but no longer PERSISTS the decrement (was `targetActor.update({shields})`, leaving shields spent forever), so the full strength applies to every attacker's fire each Round (RT Core p.226). Live-verified (shields 2 cancel 2 hits, stay at 2; a second salvo is again fully shielded). Simplification: a single attacker's multiple salvos in one turn each get full shields rather than overloading after the first.
- ✅ QA-147 (fixed 2026-06-26) — Turrets are now a purely DEFENSIVE rating (RT Core p.220), no longer firing bogus offensive percentile rolls vs Crew Rating. `rollTurrets()` posts the rating + its effects (−10/point to enemy Hit-and-Run Pilot tests, +10/point to this ship's boarding Command Test) instead of an attack. Live-verified (rating 2 → −20/+20 defensive card, no attack roll).
- ✅ QA-148 (fixed 2026-06-26) — Boarding is now a SINGLE opposed Command Test (RT Core p.219-220) between the acting ship and the targeted enemy ship: each side rolls Command (crewRating) + the modifiers (+10 per 10 Crew Population advantage, +10 per 10 Hull Integrity advantage, +10 per own turret point) via the node-tested `boardingCommandBonus()`; the loser takes 1d5 Crew + 1d5 Morale (or 1 Hull) per degree, then makes a Morale check to keep fighting or rout. Replaces the homebrew per-action d100 loop. Posts an authoritative resolution card (GM applies the losses). Live-verified on rt-smoke.
...
**data-quality** (3)
- ✅ QA-057 (fixed 2026-06-26) — 27 NPCs with `wounds.max: 0` → **26 source-extracted Wounds** set across all supplement packs (e.g. Apex Predator 15, Howling Banshee 15, Warboss Snokgritz 35, Guardian of the Inner Sanctum 45, Corrupt Wraithguard 20). The 1 remaining ("Representative", Stars of Inequity) is a non-statblock abstract entry the source tells the GM to build from the Colonist template — correctly left at 0.
- ✅ QA-058 (fixed 2026-06-26) — 3 all-zero-characteristic NPCs → **The Luminary** (51/42/42/33/51/51/42/51/60) and **Plaguebearer** (45/30/43/55/30/30/30/30/10) recovered from their stat blocks (the OCR had placed the unnatural-bonus row above the real values). "Representative" confirmed a non-statblock entry (omitted, not a bug).
- ✅ QA-059 (2026-06-26) — **76 OCR-blob weapon items FIXED → 0**: the inline weapon spec embedded in the item *name* (`vicious beak (Melee; 1d10+4; Pen 0; Primitive) # …prose`) is now parsed into the structured fields (damage/class/penetration/range/RoF/clip/reload/special) and the name cleaned (`tools/npc_pipeline/fix_blob_weapons.py` — PyYAML round-trip matching the pipeline's dump settings for minimal diffs; handles leading-class / comma-separated / `Melee/Thrown` / letterless-melee-damage / OCR-dash forms). 78 parsed + 12 prose-trimmed across 16 packs; compilePacks round-trips clean; live-verified on rt-smoke (0 blob names, `vicious beak`=1d10+4, `Slugga`=1d10+4). This also dropped empty-`damage` weapons **289 → 239**. **QA-059a fill pass DONE (2026-06-26):** source-extracted ~80 real weapon damage profiles across all 20 NPC packs via per-book subagent extraction → `tools/npc_pipeline/apply_npc_fixes.py` (per-pack JSON, minimal-diff). Empty-`damage` **289 → 160**. **The remaining 160 are NOT fillable: ~106 are junk (OCR prose fragments / cybernetics / gear mis-tagged `type: weapon`, mostly in Twilight Crusade — the real weapons aren't among them), the rest are correctly-empty (effect grenades, ammo-dependent launchers, Snare/web weapons).** **Remaining QA-059 work = a DELETION pass for the ~106 junk weapon items (cosmetic clutter; not fillable) — distinct from the fills.** **QA-059 DELETION pass DONE (2026-06-26):** `tools/npc_pipeline/classify_junk_weapons.py` removed **139 junk weapon items** (OCR prose, cybernetics/gear, clothing, mis-tagged psychic-powers) across 7 packs, keeping the **21 correctly-empty real items** (effect grenades, ammo launchers, Snare/web, special-profile weapons, placeholders) via an explicit whitelist + short-proper-noun-weapon guard (previewed for false positives first; none). 338 actors intact, 533 filled weapons preserved (diff removed/added filled-damage lines match exactly = pure git realignment), empty-`damage` weapons **160 -> 21**. **QA-059 fully resolved.**
