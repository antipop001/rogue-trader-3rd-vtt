...
**Current version: 0.7.26** (released 2026-06-05). **Generated portraits for the 158 NPCs that were shipping without one** — every NPC across all 20 packs now has a specific image. Framing audit (`tools/npc_pipeline/audit_face_framing.py` using OpenCV Haar cascades) shows 0 portraits with faces cropped *out*, but 221/334 (66%) frame the face too small for the Foundry square-token crop to capture well — those flagged in `BAD_FRAMING.md`, scheduled for a portrait-bust rerun pass.
