...
**Current version: 0.7.7** (released 2026-05-23). Weapon specials audit fixing five combat-math bugs (Storm, Lance, Razor Sharp, Unreliable, jam threshold). Traits compendium expanded from 33 → 45 with 12 Into the Storm entries. NPC compendium talents now carry full mechanical data (ActiveEffects, conditional bonus flags, pickable choices) instead of display-only stubs.
